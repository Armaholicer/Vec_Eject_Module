...
if (player == _driver) then
{
	while {player == _driver} do 
	{
		if (((FhzOfPlayer doorPhase "Door_R") == 0) AND ((FhzOfPlayer doorPhase "Door_L") == 0)) then
		{
			...
			IDHELI_DOORO = player addAction ["<t color='#FF0911'>Türen öffnen", "functions\Heli_4\open.sqf"];
			IDHELI_DOOROL = player addAction ["<t color='#000000'>Linke Tür öffnen", "functions\Heli_4\openl.sqf"];
			IDHELI_DOOROR = player addAction ["<t color='#00FF00'>Rechte Tür öffnen", "functions\Heli_4\openr.sqf"];
		...
		if (((FhzOfPlayer doorPhase "Door_R") == 1) AND ((FhzOfPlayer doorPhase "Door_L") == 1)) then
		{
			...
			IDHELI_DOORC = player addAction ["<t color='#FF0911'>Türen schließen", "functions\Heli_4\close_all.sqf"];
		...
		{
			IDHELI_DOORR = player addAction ["<t color='#FF0911'>Rechte Tür schließen", "functions\Heli_4\close_r.sqf"];			
		...
		{
			IDHELI_DOORL = player addAction ["<t color='#FF0911'>Linke Tür schließen", "functions\Heli_4\close_l.sqf"];
